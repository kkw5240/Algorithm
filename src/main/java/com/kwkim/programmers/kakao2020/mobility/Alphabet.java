...
public class Alphabet {
    public String[] solution(String[] card, String[] word) {
        ...
        for (String c : card) {
            Map<Character, Integer> lineCharCountMap = new HashMap<>();
            mapList.add(lineCharCountMap);
            for (Character ch : c.toCharArray())
                lineCharCountMap.put(ch, lineCharCountMap.getOrDefault(ch, 0) + 1);
        }

        List<String> resultList = new LinkedList<>();
        Map<Character, Integer> wordCharCountMap;
        for(String w : word) {
            wordCharCountMap = new HashMap<>();
            for (Character ch : w.toCharArray())
                wordCharCountMap.put(ch, wordCharCountMap.getOrDefault(ch, 0) + 1);

            boolean isValid = false;
            Set<Character> chSet;
            for(Map<Character, Integer> lineCharCountMap : mapList) {
                isValid = false;
                chSet = new HashSet<>(wordCharCountMap.keySet());
                for(Character ch : chSet) {
                    Integer charCount = lineCharCountMap.getOrDefault(ch, 0);
                    if(charCount == 0) {
                        continue;
                    }

                    if(charCount >= wordCharCountMap.getOrDefault(ch, 0)) {
                        wordCharCountMap.remove(ch);
                        isValid = true;
                    } else {
                        isValid = false;
                        break;
            ...
            if(isValid && wordCharCountMap.isEmpty())
                resultList.add(w);
        }
